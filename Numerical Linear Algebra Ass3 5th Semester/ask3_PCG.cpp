...
int PCG_menu();
void preconditioner_M(int, double, int, int);
void PCG_Method(double **, double **, int, int, int *, int);
...
//main of ask3_PCG.cpp

	int back2menu, n;
	double **A, *X;

	printf("\nLINEAR SYSTEM RESOLUTION OF Ax = b SYSTEMS USING PCG METHOD\n");
	...
	while(1){
		...
		switch(menu()){
			...
			case 5: printf ("\n\n\tHOPE THAT YOU ENJOYED USING MY PROGRAM FOR FINDING LINEAR SYSTEM SOLUTIONS \
					\n\tFOR SYSTEMS OF TYPE Ax = b USING THE PCG METHOD. \
...
//main menu
	...
	do{
		...
		\n\t4. File input: you are about to give a text file with your matrices in it. The file name must have the name:'ask3_PCG.txt' \
...
//menu for the 2nd selection(specific matrices) of the main manu
	...
	do{
		...
		\nSwitch the specific matrix to solve a linear system as given from exercise ask3_PCG: \
...
//data of matrix A and vectors x and b are inserted from file(extra adjustment)
	...
	if((file=fopen("ask3_PCG.txt", "r"))==NULL){
		perror("FILE ERROR : Either the file with name 'ask3_PCG.txt' does not exist or it is placed in a wrong directory");
		exit(0);			//not return 0..
	}
...
void PCG(double **A, double *x, int n){
//linear system resolution of Ax=b with PCG method

	double cputime;
    struct timeval t1, t2;
	int i, j, ch, itmax, itcount=0;
	...
	printf("\n\nExecution of PCG method for linear system Ax = b resolution started...\n\n");
	gettimeofday(&t1, NULL);
	PCG_Method(A, &x, n, itmax, &itcount, ch);
	gettimeofday(&t2, NULL);
	printf("\n\nExecution of PCG method for linear systems Ax = b resolution ended...\n");
	...
	printf("\nExecution time of PCG system : %.4lf ms\n\n", cputime);
...
int PCG_menu(){
//Preconditioner matrix menu

	int menuSelection;
	do{
		printf ("PRECONDITIONER MATRIX MENU \
		\nSwitch the type of preconditioner matrix you want to use with the PCG method: \
		\n\t1. Jacobi Preconditioner Matrix \
		\n\t2. Cholesky Preconditioner Matrix \
		\n\t3. SOR preconditioner Matrix \
		\n\t4. Select if you want to quit.\n");
		printf("INPUT: ");
		scanf("%d", &menuSelection);
...
void preconditioner_M(double **A, double ***M, int n, int ch){
//computation of a preconditioner matrix 
	...
	case 2: (*M)[0][0] = sqrt(fabs(A[0][0]));
			for(i=1 ; i<n ; i++)
				(*M)[i][0] = ((double)A[i][0])/((double)(*M)[0][0]);
			sum = 0.0;
			for(i=1 ; i<n-1 ; i++){
				for(j=0 ; j<i ; j++)
					sum += (*M)[i][j]*(*M)[i][j];

				(*M)[i][i] = sqrt(fabs(A[i][i]-sum));
				sum = 0.0;
				for(k=i+1 ; k<n ; k++){
					for(j=0 ; j<i ; j++)
						sum += (*M)[k][j]*(*M)[i][j];

					(*M)[k][i] = ((double)(A[k][i]-sum))/((double)(*M)[i][i]);
					sum = 0.0;
				}
			}
			for(j=0 ; j<n-1 ; j++)
			...
			(*M)[n-1][n-1] = sqrt(fabs(A[n-1][n-1]-sum));
			break;

        case 3:	printf("Now give the w parameter in 0.1(0.1)1.9 (e.g. w=0.4) :\n");
				printf("w = ");
				scanf("%lf", &w);
...
void PCG_Method(double **A, double **u, int n, int itmax, int *itcount, int ch){
//algorithm of PCG

	long double e = 0.0000005;
	double **M, *Au, *r, *p, *w, *y, a=0.0, b=0.0, sum=0.0, yr_mult=0.0, yr_mult2, r_norm, pw_mult;
	int i, j;


	//M, Au, r, p, w, r, s matrix allocations
    ...
    switch(ch){
        ...
        case 2:
		case 3:	y[0] = ((double)r[0])/((double)M[0][0]);
				for(i=1 ; i<n ; i++){
					for(j=0 ; j<i ; j++)
						sum += M[i][j]*y[j];
					y[i] = ((double)(r[i]-sum))/((double)M[i][i]);
					p[i] = y[i];
					yr_mult += y[i]*r[i];
					sum = 0.0;
				}
	...
	for( ; (*itcount)<itmax ; (*itcount)++){
		for(i=0 ; i<n ; i++){
			w[i] = 0.0;
			for(j=0 ; j<n ; j++)
				w[i] += ((double)A[i][j]*p[j]);
		}
		pw_mult = 0.0;
		for(i=0 ; i<n ; i++)
			pw_mult += ((double)p[i]*w[i]);

		a = ((double)yr_mult)/((double)pw_mult);
		for(i=0 ; i<n ; i++){
			(*u)[i] += ((double)a*p[i]);
			r[i] -= ((double)a*w[i]);
			//printf("--%lf--",(*u)[i]);
		...
		if(r_norm < e)
			...
			yr_mult2 = 0.0;
			for(i=0 ; i<n ; i++)
				yr_mult2 += ((double)y[i]*r[i]);
			b = ((double)yr_mult2)/((double)yr_mult);
			yr_mult = yr_mult2;
			for(i=0 ; i<n ; i++)
				p[i] = y[i]+((double)b*p[i]);
		}
	}
