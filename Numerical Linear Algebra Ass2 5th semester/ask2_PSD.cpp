...
//main of ask2_PSD.cpp

	int back2menu, n;
	double **A, *X;

	printf("\nLINEAR SYSTEM RESOLUTION OF Ax = b SYSTEMS USING PSD ITERATIVE METHOD\n");
	...
	while(1){
		...
		switch(menu()){
			...
			case 5: printf ("\n\n\tHOPE THAT YOU ENJOYED USING MY PROGRAM FOR FINDING LINEAR SYSTEM SOLUTIONS \
					\n\tFOR SYSTEMS OF TYPE Ax = b USING THE PSD ITERATIVE METHOD. \
...
//main menu
	...
	do{
		...
		\n\t4. File input: you are about to give a text file with your pentadiagonial matrices in it. The file name must have the name:'ask2_PSD.txt' \
...
//menu for the 2nd selection(specific matrices) of the main manu
	...
	do{
		...
		\nSwitch the specific matrix to solve a linear system as given from exercise ask2_PSD: \
...
void PSD(double **A, double *x, int n){
//linear system resolution of Ax=b with PSD iterative method
...
//	x = (double*)malloc(n*sizeof(double));
	...
	switch(PSD_menu()){
		case 1:	printf("Now give the t and w parameters respectively in this order with t, w in 0.1(0.1)1.9 (e.g. t=0.1, w=0.4) :\n");
				...
				printf("\n\nExecution of PSD method for linear system Ax = b resolution started...\n\n");

				PSD_iterativeMethod(A, &x, n, t, w, itmax, &itcount);

				gettimeofday(&t2, NULL);
				printf("\n\nExecution of PSD iterative method for linear systems Ax = b resolution ended...\n");
				...
				printf("\nExecution time of PSD system : %.4lf ms\n\n", cputime);
		...
		printf("The parameters t and w are iteratively selected in range [0.1, 1.9] in oder to find the optimal values of t and w\n");
				printf("\n\nExecution of PSD method for linear system Ax = b resolution with all the values of t and w in range started...\n\n");
				...
				printf("\nExecution of PSD iterative method for linear systems Ax = b resolution with all the values of t and w in range ended...\n");
				...
				PSD_iterativeMethod(A, &x, n, t_optimal, w_optimal, itmax, &min_itcount);
				...
				printf("\nExecution of PSD method for linear system Ax = b resolution with all the values of t and w in range took : %.4lf ms\n\n", cputime);
				printf("\nExecution time of PSD system with the minimum number of iterations took : %.4lf ms\n\n", best_cputime);
				break;
...
//main menu
	...
	do{
		...
		\nSwitch how you want to use the PSD iterative method: \
		\n\t1. Experimental verification of correctness of PSD algorithm for resolution of the linear system Ax=b with x given x=(1,1,...,1)^T \
		\n\t2. Experimental study of convergence of the PSD iterative method with t, w given in [0.1, 1.9] with step 0.1 \
...
//algorithm of PSD for factorizing matrix A in L and U using partial pivoting
	...
	//xo, x1, Ux0, Ux1, k, l, d, r, s matrix allocations
	...
	//initialization of  x0, x1, Ux0, Ux1, k, l, r, s
	for(i=0 ; i<n ; i++){
			x0[i] = 0;
			x1[i] = 0;
			Ux0[i] = 0;
	...
	while((*itcount) < itmax){
		...
		//step B4.4 - assign x to x0 to aprroximate better in next iteration
			for(i=0 ; i<n ; i++)
				x0[i] = (*x)[i];
	}
	free(x0);
